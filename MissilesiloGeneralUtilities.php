...
/**
 * A class used to handle basics stuff
 * @name MissilesiloGeneralUtilities
 * @package Missilesilo General Library
 * @subpackage Utilities
 * @author Missilesilo
 * @version 1.0
 */
class MissilesiloGeneralUtilities
{
	private $log_location;
	private $allow_write_to_log = true; //TODO: Turn this off when live
	private $db_log_queries = true;
	private $db_throw_exceptions = true;
	private $db_use_query_cache = false;
	private $db_cache_text = null;
	
	private $db_credentials_are_okay = false;
	...
	public function __construct(MissilesiloMySQLCredentials $custom_credentials = null, $log_location = './ms_general_utilities.log')
	{
		$this->log_location = $log_location;
		if(!strlen($this->log_location)) $this->log_location = './ms_general_utilities.log';
		
		if($this->db_use_query_cache) $this->db_cache_text = 'SQL_CACHE ';
		
		if($custom_credentials !== null){
			$this->db_host = $custom_credentials->getHost();
			$this->db_name = $custom_credentials->getDB();
			$this->db_user = $custom_credentials->getUser();
			$this->db_pass = $custom_credentials->getPass();
			$this->custom_loaded = true;
		}
	
		if(!$this->custom_loaded){
			//Try the standard definitions of wp database information
			if($this->db_host === null && defined('DB_HOST')) 		$this->db_host = DB_HOST;
			if($this->db_name === null && defined('DB_NAME')) 		$this->db_name = DB_NAME;
			if($this->db_user === null && defined('DB_USER')) 		$this->db_user = DB_USER;
			if($this->db_pass === null && defined('DB_PASSWORD')) 	$this->db_pass = DB_PASSWORD;
		}
		
		if($this->db_host !== null && $this->db_name !== null && $this->db_user !== null) $this->db_credentials_are_okay = true;
	}
	
	
	/**
	 * Gets an instance of PDO based on the stored parameters
	 * @param string $create_new_instance
	 * @return PDO
	 */
	...
	{	
		...
		if(!$this->db_credentials_are_okay){
			throw new Exception('There was a problem with the db credentials');
			return null;
		}
	...
	{
		if(!$this->db_log_queries) return $results;
	...
	public function select_all_distinct_by($table, $field, array $where = [], $orderby = null, $asc_desc = 'ASC')
	{
		...
		$query = "SELECT{$this->db_cache_text} DISTINCT `$field` FROM `$table`{$wherear['query']}$ordertext";
	...
	public function select_one_by($table, $field, $value, $orderby = null, $asc_desc = 'ASC')
	{
		...
		$query = "SELECT{$this->db_cache_text} * FROM `$table` WHERE `$field` = :id LIMIT 1";
	...
	public function select_one_value_by($table, $field, array $where = [], $orderby = null, $asc_desc = 'ASC')
	{
		...
		$query = "SELECT{$this->db_cache_text} `$field` FROM `$table`{$wherear['query']}$ordertext LIMIT 1";
	...
	public function select_all_by($table, $field, $value, $orderby = null, $asc_desc = 'ASC')
	{
		...
		$query = "SELECT{$this->db_cache_text} * FROM `$table` WHERE `$field` = :id$ordertext";
	...
	public function select_all_order($table, $orderby, $asc_desc = 'ASC')
	{
		...
		$query = "SELECT{$this->db_cache_text} * FROM `$table` ORDER BY $orderby $asc_desc";
	...
	public function count_by($table, $field, $value)
	{
		...
		$query = "SELECT{$this->db_cache_text} COUNT(*) FROM `$table` WHERE `$field` = :id";
		$statement = $this->run_raw_query_and_return_statement($query, ['id' => $value]);
	...
	public function count_by_multi($table, array $where = [], $orderby = null, $asc_desc = 'ASC')
	{
		...
		$query = "SELECT{$this->db_cache_text} COUNT(*) FROM `$table`{$wherear['query']}$ordertext";
		$statement = $this->run_raw_query_and_return_statement($query, sizeof($wherear['array'])?$wherear['array']:null);
		return $statement->fetchAll(PDO::FETCH_COLUMN, 0);
	}
	...
	{
		return $this->redirect_to_url_raw($this->base_url(), 302);
	}
	
	/**
	 * Convenience method for constructing and sanitizing a url
	...
	/**
	 * Sanitizes a url and removes nasty bad characters
	 * @param string $url
	 */
	...
	{
		if(!is_array($_SERVER)){ 
			if($throw_exception_when_cant_be_searched) throw new Exception('$_SERVER is not an array');
			return null;
		} 
	...
	{
		if(!is_array($_GET)){
			if($throw_exception_when_cant_be_searched) throw new Exception('$_GET is not an array');
			return null;
		}
	...
	{
		if(!is_array($_POST)){
			if($throw_exception_when_cant_be_searched) throw new Exception('$_POST is not an array');
			return null;
		}
			...
			$throw_exception_when_cant_be_searched = MISSILESILO_GENERAL_UTILITIES__DEFAULT_THROW_EXCEPTION_IN_REQUEST_SEARCH)
	{
		...
		if(!is_array($_SESSION)){
			if($throw_exception_when_cant_be_searched) throw new Exception('$_SESSION is not an array');
			return null;
		}
	 ...
	 * @return Ambiguous (seriously, it could be anything you can store in an array)
	 */
	...
	{
		if($absolute)
			return realpath($this->log_location);
		else
			$this->log_location;
	}
	...
	{
		if(!$this->allow_write_to_log) return null;
		$fp = fopen($this->log_location, 'r');
	...
	public function write_to_log($line)
	{
		...
		return file_put_contents(
				$this->log_location,							//File to write to
				date(DATE_ATOM)."\t$line\n",					//Line to write
