...
class DB extends AbstractUtility
{
    ...
    public function getPDO($create_new_instance = true)
    {
        ...
        return ($passwd == ''
            ?$this->pdo_instance = new \PDO("{$this->config->db_type}:host={$host}; dbname={$dbname}; charset=utf8", $username, null, $options)
            :$this->pdo_instance = new \PDO("{$this->config->db_type}:host={$host}; dbname={$dbname}; charset=utf8", $username, $passwd, $options));
    }
